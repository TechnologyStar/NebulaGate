...
func SetApiRouter(router *gin.Engine) {
    ...
    apiRouter.Use(middleware.GlobalAPIRateLimit())
    {
        ...
        planAdminRoute := apiRouter.Group("/plan")
        planAdminRoute.Use(middleware.AdminAuth())
        {
            planAdminRoute.GET("/", controller.GetAllPlans)
            planAdminRoute.GET("/:id", controller.GetPlan)
            planAdminRoute.POST("/", controller.CreatePlan)
            planAdminRoute.PUT("/:id", controller.UpdatePlan)
            planAdminRoute.DELETE("/:id", controller.DeletePlan)
            planAdminRoute.POST("/assign", controller.AssignPlan)
            planAdminRoute.DELETE("/assignment/:id", controller.DetachPlanAssignment)
            planAdminRoute.GET("/assignment/:assignment_id/usage", controller.GetPlanUsage)
        }

        planSelfRoute := apiRouter.Group("/plan/self")
        planSelfRoute.Use(middleware.UserAuth())
        {
            planSelfRoute.GET("/", controller.GetUserPlans)
        }

        voucherRoute := apiRouter.Group("/voucher")
        {
            voucherAdminRoute := voucherRoute.Group("/")
            voucherAdminRoute.Use(middleware.AdminAuth())
            {
            ...
            voucherUserRoute.Use(middleware.UserAuth())
            {
