...
func (channel *Channel) GetNextEnabledKey() (string, int, *types.NewAPIError) {
    ...
    case constant.MultiKeyModePolling:
        ...
        // If no enabled key found, fallback to first enabled
        if selectedIdx == -1 {
            selectedIdx = enabledIdx[0]
        }
        
        // Update polling index for next call
        nextPollingIndex := (selectedIdx + 1) % len(keys)
        ...
        if common.DebugEnabled {
            println(fmt.Sprintf("channel %d polling: selected key %d, next index: %d", channel.Id, selectedIdx, nextPollingIndex))
        }
