...
func TestUsageCounterIncrement(t *testing.T) {
	...
	if err := IncrementUsageCounter(a.Id, p.QuotaMetric, 5, start, end); err != nil { t.Fatalf("increment: %v", err) }
	if err := IncrementUsageCounter(a.Id, p.QuotaMetric, 7, start, end); err != nil { t.Fatalf("increment2: %v", err) }
	var c UsageCounter
	if err := db.Where("plan_assignment_id = ? AND metric = ? AND cycle_start = ?", a.Id, p.QuotaMetric, start).First(&c).Error; err != nil { t.Fatalf("load: %v", err) }
	if c.ConsumedAmount != 12 { t.Fatalf("expected 12, got %d", c.ConsumedAmount) }
}
