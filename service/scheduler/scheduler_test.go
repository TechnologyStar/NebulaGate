...
func TestPlanResetJob(t *testing.T) {
	...
	// Run reset sweep
	if err := RunPlanCycleResetOnce(nil); err != nil {
		t.Fatalf("reset once: %v", err)
	}
...
func TestTTLCleanupFlags(t *testing.T) {
	...
	if err := RunTTLCleanupOnce(nil); err != nil {
...
func TestCarryOverRules(t *testing.T) {
	...
	if err := db.Create(a1).Error; err != nil {
		t.Fatalf("create assignment1: %v", err)
	}
	...
	if err := db.Create(c1).Error; err != nil {
		t.Fatalf("create counter1: %v", err)
	}

	if err := RunPlanCycleResetOnce(nil); err != nil {
		t.Fatalf("reset once case1: %v", err)
	}

	var a1After model.PlanAssignment
	if err := db.First(&a1After, a1.Id).Error; err != nil {
	...
	if err := db.Create(c2).Error; err != nil {
		t.Fatalf("create counter2: %v", err)
	}

	if err := RunPlanCycleResetOnce(nil); err != nil {
		t.Fatalf("reset once case2: %v", err)
	}

	var a2After model.PlanAssignment
	if err := db.First(&a2After, a2.Id).Error; err != nil {
