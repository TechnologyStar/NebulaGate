...
func createPlanAndAssignment(t *testing.T, db *gorm.DB, userId int, quotaAmount int64, billingMode string, fallback bool) (*model.Plan, *model.PlanAssignment) {
	...
	if billingMode != "" {
		assignment.BillingMode = billingMode
	}
...
func TestBillingEngine_PlanOnly(t *testing.T) {
	...
	if pc.Mode != common.BillingModePlan {
		t.Fatalf("expected plan mode, got %s", pc.Mode)
	}
	if pc.Allowance != 50 {
		t.Fatalf("expected allowance 50, got %d", pc.Allowance)
	}
	...
	if counter.ConsumedAmount != 10 {
		t.Fatalf("expected consumed 10, got %d", counter.ConsumedAmount)
	}
	...
	if counter2.ConsumedAmount != 10 {
		t.Fatalf("expected consumed still 10 after retry, got %d", counter2.ConsumedAmount)
	}
}
...
func TestBillingEngine_BalanceOnly(t *testing.T) {
	...
	if pc.Mode != common.BillingModeBalance {
		t.Fatalf("expected balance mode, got %s", pc.Mode)
	}
	reqId := "req-balance-1"
	...
	if u.Quota != 80 {
		t.Fatalf("expected user quota 80, got %d", u.Quota)
	}
	// Retry idempotent
	_, err = engine.CommitCharge(nil, &CommitParams{Prepared: pc, Amount: 20, RequestId: reqId, RelayInfo: newRelayInfo(user.Id, token.Id, true)})
	if err != nil {
	...
	var u2 model.User
	if err := db.First(&u2, user.Id).Error; err != nil {
		t.Fatalf("load user: %v", err)
	}
	if u2.Quota != 80 {
		t.Fatalf("expected user quota remains 80, got %d", u2.Quota)
	}
}
...
func TestBillingEngine_AutoFallback(t *testing.T) {
	...
	if pc.Mode != common.BillingModePlan {
		t.Fatalf("expected plan mode at prepare, got %s", pc.Mode)
	}
	...
	if cnt != 0 {
		t.Fatalf("expected plan usage 0 due to fallback, got %d", cnt)
	}
	var u model.User
	if err := db.First(&u, user.Id).Error; err != nil {
		t.Fatalf("load user: %v", err)
	...
	if u.Quota != 90 {
		t.Fatalf("expected user quota 90 after fallback, got %d", u.Quota)
	}
}
...
func TestBillingEngine_ConcurrencyGuard(t *testing.T) {
	...
	if u.Quota != 70 {
		t.Fatalf("expected single deduction to 70, got %d", u.Quota)
	}
}
