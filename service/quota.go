...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
    ...
    // Billing-aware commit
    if common.BillingFeatureEnabled {
        if preparedAny, ok := ctx.Get("billing_prepared"); ok {
            if prepared, ok2 := preparedAny.(*PreparedCharge); ok2 {
                engine := NewBillingEngine(nil)
                requestId := ctx.GetString(common.RequestIdKey)
                upstream := strings.ToLower(constant.GetChannelTypeName(ctx.GetInt("channel_type")))
                _, err := engine.CommitCharge(ctx, &CommitParams{
                    Prepared:         prepared,
                    Amount:           int64(quota),
                    RequestId:        requestId,
                    ...
                    Upstream:         upstream,
                    UsageMetric:      "",
                    PromptTokens:     int64(textInputTokens),
                    CompletionTokens: int64(textOutTokens),
                    LatencyMs:        (time.Now().UnixMilli() - relayInfo.StartTime.UnixMilli()),
                    RelayInfo:        relayInfo,
                })
                if err != nil {
                    logger.LogError(ctx, "billing commit failed: "+err.Error())
                }
                logContent = fmt.Sprintf("计费模式：%s", prepared.Mode)
            }
        }
    }
    ...
    // annotate billing metadata for legacy log
    if common.BillingFeatureEnabled {
        other["billing_mode"] = relayInfo.BillingMode
        if relayInfo.PlanAssignmentId != 0 {
            other["plan_assignment_id"] = relayInfo.PlanAssignmentId
        }
        if relayInfo.PlanLabel != "" {
            other["plan_label"] = relayInfo.PlanLabel
        }
    }
...
func PostAudioConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
        ...
        if preparedAny, ok := ctx.Get("billing_prepared"); ok {
            if prepared, ok2 := preparedAny.(*PreparedCharge); ok2 {
                ...
                _, err := engine.CommitCharge(ctx, &CommitParams{
                    ...
                    Amount:           int64(quota),
                    RequestId:        requestId,
                    ModelAlias:       relayInfo.OriginModelName,
                    Upstream:         upstream,
                    UsageMetric:      "",
                    PromptTokens:     int64(textInputTokens),
                    CompletionTokens: int64(textOutTokens),
    ...
    if quotaDelta > 0 && !common.BillingFeatureEnabled {
        logger.LogInfo(ctx, fmt.Sprintf("预扣费后补扣费：%s（实际消耗：%s，预扣费：%s）",
            logger.FormatQuota(quotaDelta),
            logger.FormatQuota(quota),
            logger.FormatQuota(relayInfo.FinalPreConsumedQuota),
        ))
    } else if quotaDelta < 0 && !common.BillingFeatureEnabled {
        logger.LogInfo(ctx, fmt.Sprintf("预扣费后返还扣费：%s（实际消耗：%s，预扣费：%s）",
            logger.FormatQuota(-quotaDelta),
            logger.FormatQuota(quota),
            logger.FormatQuota(relayInfo.FinalPreConsumedQuota),
