...
// RedisIncrByOne increments a key by 1
func RedisIncrByOne(key string) error {
    if !RedisEnabled {
        return errors.New("Redis is not enabled")
    }
    
    if DebugEnabled {
        SysLog(fmt.Sprintf("Redis INCRBYONE: key=%s", key))
    }
...
// RedisExpire sets expiration for a key
func RedisExpire(ctx context.Context, key string, expiration time.Duration) error {
    ...
    _, err := RDB.Expire(ctx, key, expiration).Result()
    return err
}

// RedisScan scans for keys matching a pattern
func RedisScan(ctx context.Context, pattern string) ([]string, error) {
    if !RedisEnabled {
    ...
    for {
        scanKeys, nextCursor, err := RDB.Scan(ctx, cursor, pattern, 100).Result()
        if err != nil {
            return nil, err
        }
