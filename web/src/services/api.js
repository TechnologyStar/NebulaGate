/*
Copyright (C) 2025 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as
published by the Free Software Foundation, either version 3 of the
License, or (at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact support@quantumnous.com
*/

import { API } from '../helpers';

export const fetchPlans = async () => {
  const response = await API.get('/api/plan');
  return response.data;
};

export const createPlan = async (payload) => {
  const response = await API.post('/api/plan', payload);
  return response.data;
};

export const updatePlan = async (planId, payload) => {
  const response = await API.put(`/api/plan/${planId}`, payload);
  return response.data;
};

export const deletePlan = async (planId) => {
  const response = await API.delete(`/api/plan/${planId}`);
  return response.data;
};

export const generateVoucher = async (payload) => {
  const response = await API.post('/api/voucher/batch', payload);
  return response.data;
};

export const listVouchers = async () => {
  const response = await API.get('/api/voucher/batch');
  return response.data;
};
