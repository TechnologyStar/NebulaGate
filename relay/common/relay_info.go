...
type RelayInfo struct {
    ...
    // Billing context snapshot for logging and UI
    BillingFeatureEnabled bool
    BillingMode           string
...
func genBaseRelayInfo(c *gin.Context, request dto.Request) *RelayInfo {
    ...
    // Hydrate billing context snapshot for downstream / logging convenience
    info.BillingFeatureEnabled = common.GetContextKeyBool(c, constant.ContextKeyBillingFeatureEnabled)
    if info.BillingFeatureEnabled {
        mode := common.GetContextKeyString(c, constant.ContextKeyBillingMode)
        if mode == "" {
            mode = common.BillingDefaultMode
        }
        info.BillingMode = mode
        info.PlanAssignmentId = common.GetContextKeyInt(c, constant.ContextKeyPlanAssignmentID)
        info.PlanLabel = common.GetContextKeyString(c, constant.ContextKeyPlanLabel)
    }
